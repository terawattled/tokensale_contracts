...
contract('Crowdsale', (accounts) => {
  ...
  beforeEach(async function() {
    ...
    ledToken = await LedToken.new(ledPresaleTokenAddress)
    ledTokenAddress = await getAddress(ledToken)

    tokenSale = await TokenSale.new(
                wallet,
                ledPresaleTokenAddress,
                ledTokenAddress,
                startBlock,
                endBlock,
                ledWalletAddress)
  ...
  describe('Initial State', function () {
    beforeEach(async function() {
      transferOwnership(ledToken, fund, tokenSaleAddress)
    })
    ...
    it('should initially be linked to the Led Presale token', async function() {
      let tokenSalePresaleToken = await tokenSale.ledPresaleToken.call()
      tokenSalePresaleToken.should.be.equal(ledPresaleTokenAddress)
    })
  ...
  describe('End and Beginning of Presale', async function() {
    beforeEach(async function() {
      await transferOwnership(ledToken, fund, tokenSaleAddress)
    })
  ...
  describe('Crowdsale', async function() {
    ...
    it('should increase total token supply', async function() {
      let initialTotalSupply = await getTotalSupply(ledToken)
      ...
      supplyIncrease = await baseUnits(ledToken, supplyIncrease)
    ...
    it('should create tokens for the sender', async function() {
      let initialTokenBalance = await getTokenBalance(ledToken, sender)

      await buyTokens(tokenSale, sender, 1 * ether)

      let tokenBalance = await getTokenBalance(ledToken, sender)
      let balanceIncrease = await baseUnits(ledToken, tokenBalance - initialTokenBalance)

      let expectedBalanceIncrease = await numberOfTokensFor(tokenSale, 1 * ether)
      expectedBalanceIncrease = await baseUnits(ledToken, expectedBalanceIncrease)
    ...
    it('should increase buyer balance by X for 10 ether invested', async function() {
      ...
      balanceIncrease = await baseUnits(ledToken, balanceIncrease)
